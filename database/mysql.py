...
class MySQLDatabase(object):
    ...
    def select(self, table, columns=None, named_tuples=False, **kwargs):
        ...
        if not columns:
            ...
            sql_str = sql_str[:-2] # removes the last comma
        ...
        # if there's a ORDER BY ASC clause attached
        if kwargs.has_key('order_asc'):
            sql_str += " ORDER BY %s ASC" % kwargs.get('order_asc')

        # if there's a ORDER BY DESC clause attached
